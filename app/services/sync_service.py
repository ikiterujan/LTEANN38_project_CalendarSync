    ...
    async def sync_user_from_master(self, db: Session, user_id: str) -> dict:
        ...
        try:
            ...
            for item in master_items:
                # target_grades를 문자열/정수 모두 대응되도록 정수로 통일 추출
                target_grades = []
                if item.grade1: target_grades.append(1)
                if item.grade2: target_grades.append(2)
                if item.grade3: target_grades.append(3)

                # 유저 학년 데이터 안전하게 정수로 변환 후 비교
                user_grade_int = int(user.grade) if str(user.grade).isdigit() else None

                # 학년 지정이 존재하고, 유저 학년이 타겟 학년에 안 들면 패스
                # (target_grades가 아예 비어있다면 전체 대상공지로 간주)
                if target_grades and user_grade_int not in target_grades:
                    continue
                ...
                if not existing_evt_id:
                    ...
                    # [UPDATE] 이미 존재하는 일정 갱신 (선택 사항)
                    if hasattr(self, "_update_single_user_event"):
                        await self._update_single_user_event(
                            user_id=user.id,
                            user_grade=user.grade,
                            master_id=item.id,
                            existing_event_id=existing_evt_id,
                            formatted_title=item.title,
                            start_dt=item.start_datetime,
                            end_dt=item.end_datetime,
                            location=item.location,
