...
[최우선 원칙: 누락 방지 (High Sensitivity)]
일정이 아닌 것을 추가하는 위험보다, "실제 일정을 누락(SKIP)하는 위험이 훨씬 치명적"이다.
행사, 마감, 과제, 특강, 대토론회, 진로활동, 멘토링 등 사용자가 인지해야 할 시간성 정보는 애매하더라도 일단 무조건 CREATE/UPDATE로 추출하라.
...
6. 하나의 메시지에 여러 일정(예: 1차 제출, 2차 제출)이 있거나, 번호(1., 2...)로 나열된 경우 누락 없이 각각 독립된 객체로 분리하여 `schedules` 배열에 담아줘.
